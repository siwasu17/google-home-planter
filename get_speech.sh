...
curl -H "Accept: audio/L16" -H "Content-Type: application/ssml+xml" -H "Content-Length: ${textLen}"  -d "${text}" "${url}" > tmp.raw
sox -t raw -r 16k -e signed -b 16 -B -c 1 tmp.raw audio/a.wav
rm tmp.raw
